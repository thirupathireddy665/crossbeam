...
class IntMultiply(Int):
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(IntMultiply.name()) != size:
                continue
...
class IntModulo(Int):
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(IntModulo.name()) != size:
                continue
...
class BoolLessThan(Bool):
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(BoolLessThan.name()) != size:
                continue
            layer1_prog = plist.get_programs(layer1, INT_TYPES['type'])
            layer2_prog = plist.get_programs(layer2, INT_TYPES['type'])
            for prog1 in layer1_prog:
                for prog2 in layer2_prog:
                    yield BoolLessThan(prog1, prog2)


# Boolean classes and terminals

BOOL_TYPES = {'type': 'boolean', 'classes': (BoolLiteral, BoolEqual, BoolContain,
                                             BoolSuffixof, BoolPrefixof, BoolGreaterThan, BoolLessThan)}
