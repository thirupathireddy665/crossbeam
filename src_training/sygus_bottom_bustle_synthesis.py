...
class ProgramList:
    ...
    def __init__(self, string_variables_list, integer_variables_list, input_output):
        ...
        for string_variable in string_variables_list:
            for BottomInputStringOutputStringProperty in BottomInputStringOutputStringProperties:
                property_value = BottomInputStringOutputStringProperty(self.parent_input_output, string_variable)
        ...
        for padding_index in range(0, max_string_variables - len(string_variables_list)):
            for _ in BottomInputStringOutputStringProperties:
        ...
        for job_index in range(0, total_jobs, batch_size):
            ...
            for cur_program in current_batch:
                ...
                if cur_program.getReturnType() == STR_TYPES['type']:
                    for BottomInputStringOutputStringProperty in BottomInputStringOutputStringProperties:
                        property_value = BottomInputStringOutputStringProperty(child_input_outputs, 'cout')
    ...
    def grow(self, operations, size):
        ...
        for op in operations:
            for new_program in op.grow(self.plist, size):
                # print(p.toString)
                if (new_program is not None) and new_program.toString() not in self.closed_list and not self.has_equivalent(new_program):
...
if __name__ == "__main__":
    ...
    TaskId = None
    log_filename = logs_directory + "sygus_bottom_bustle_output.log"
    ...
    for count, filename in enumerate(benchmarks):
        ...
        if TaskId is not None and TaskId-1 != count:
            continue
        ...
        p, num = synthesizer.synthesize(40, dsl_functions,
                                        string_literals,
                                        integer_literals,
                                        [True, False],
                                        string_variables,
        ...
        if p is not None:
            ...
            logging.info("Program: " + p.toString())
