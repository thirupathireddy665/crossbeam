...
# properties additional classes
class IntStrCount(Int):
    ARITY = 2

    def __init__(self, super_str, sub_str):
        self.super_str = super_str
        self.sub_str = sub_str
        self.id = BustlePCFG.get_instance().get_program_id()
        self.size = super_str.size + sub_str.size + BustlePCFG.get_instance().get_cost(self)

    def toString(self):
        return self.super_str.toString() + ".count(" + self.sub_str.toString() + ")"

    def interpret(self, env):
        try:
            return self.super_str.interpret(env).count(self.sub_str.interpret(env))
        except:
            pass
        return None
    ...
    def getProgramIds(self, programIds):
        programIds.add(self)
        self.super_str.getProgramIds(programIds)
        self.sub_str.getProgramIds(programIds)
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(IntStrCount.name()) != size:
                continue
            ...
            for prog1 in layer1_prog:
                for prog2 in layer2_prog:
                    yield IntStrCount(prog1, prog2)


class IntLastIndexOf(Int):
    ARITY = 2
    ...
    def toString(self):
        return self.input_str.toString() + ".LastIndexOf(" + self.substr.toString() + ")"
    ...
    def interpret(self, env):
        ...
        index = None
        try:
            index = super_string.rindex(sub_string)
        except:
            pass
        return index
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(IntLastIndexOf.name()) != size:
                continue
            layer1_prog = plist.get_programs(layer1, STR_TYPES['type'])
            layer2_prog = plist.get_programs(layer2, STR_TYPES['type'])
            ...
            for prog1 in layer1_prog:
                ...
                for prog2 in layer2_prog:
                    if isinstance(prog2, StrLiteral) and prog2.toString() == EMPTY_STRING:
                        continue
                    yield IntLastIndexOf(prog1, prog2)
...
class BoolGreaterThanEqual(Bool):
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            ...
            for prog1 in layer1_prog:
                for prog2 in layer2_prog:
                    yield BoolGreaterThanEqual(prog1, prog2)


# Properties additional bool classes
class BoolAnd(Bool):
    ARITY = 2
    ...
    def toString(self):
        return "And(" + self.left.toString() + "," + self.right.toString() + ")"

    def interpret(self, env):
        try:
            return True if self.left.interpret(env) and self.right.interpret(env) else False
        except:
            pass
        return None
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(BoolAnd.name()) != size:
                continue
            layer1_prog = plist.get_programs_all(layer1)
            layer2_prog = plist.get_programs_all(layer2)
            for prog1 in layer1_prog:
                for prog2 in layer2_prog:
                    if (
                            (not isinstance(prog1, BOOL_TYPES['classes'])) or (not isinstance(prog2,
                                                                                            BOOL_TYPES['classes']))
                    ):
                        yield BoolAnd(BoolLiteral(True), BoolLiteral(False))
                    else:
                        yield BoolAnd(prog1, prog2)
...
class BoolOr(Bool):
    ...
    def toString(self):
        return "Or(" + self.left.toString() + "," + self.right.toString() + ")"

    def interpret(self, env):
        try:
            return True if self.left.interpret(env) or self.right.interpret(env) else False
        except:
            pass
        return None
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1, layer2 = combination
            if layer1 + layer2 + BustlePCFG.get_instance().get_cost_by_name(BoolOr.name()) != size:
                continue
            layer1_prog = plist.get_programs_all(layer1)
            layer2_prog = plist.get_programs_all(layer2)
            for prog1 in layer1_prog:
                for prog2 in layer2_prog:
                    if (
                        ...
                        yield BoolOr(BoolLiteral(True), BoolLiteral(False))
                    else:
                        yield BoolOr(prog1, prog2)


class BoolIsAlNum(Bool):
    ARITY = 1
    ...
    def toString(self):
        return self.str.toString() + ".isalnum()"

    def interpret(self, env):
        try:
            return self.str.interpret(env).isalnum()
        except:
            pass
        return None
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1 = combination
            if layer1 + BustlePCFG.get_instance().get_cost_by_name(BoolIsAlNum.name()) != size:
                continue
            layer1_prog = plist.get_programs(layer1, STR_TYPES['type'])

            for prog1 in layer1_prog:
                yield BoolIsAlNum(prog1)


class BoolIsDigit(Bool):
    ARITY = 1
    ...
    def toString(self):
        return self.str.toString() + ".isdigit()"

    def interpret(self, env):
        try:
            return self.str.interpret(env).isdigit()
        except:
            pass
        return None
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1 = combination
            if layer1 + BustlePCFG.get_instance().get_cost_by_name(BoolIsDigit.name()) != size:
                continue
            layer1_prog = plist.get_programs(layer1, STR_TYPES['type'])

            for prog1 in layer1_prog:
                yield BoolIsDigit(prog1)


class BoolIsLower(Bool):
    ARITY = 1
    ...
    def toString(self):
        return self.str.toString() + ".islower()"

    def interpret(self, env):
        try:
            return self.str.interpret(env).islower()
        except:
            pass
        return None
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1 = combination
            if layer1 + BustlePCFG.get_instance().get_cost_by_name(BoolIsLower.name()) != size:
                continue
            layer1_prog = plist.get_programs(layer1, STR_TYPES['type'])

            for prog1 in layer1_prog:
                yield BoolIsLower(prog1)


class BoolIsUpper(Bool):
    ARITY = 1
    ...
    def toString(self):
        return self.str.toString() + ".isupper()"

    def interpret(self, env):
        try:
            return self.str.interpret(env).isupper()
        except:
            pass
        return None
    ...
    def grow(plist, size):
        ...
        for combination in combinations:
            layer1 = combination
            if layer1 + BustlePCFG.get_instance().get_cost_by_name(BoolIsUpper.name()) != size:
                continue
            layer1_prog = plist.get_programs(layer1, STR_TYPES['type'])

            for prog1 in layer1_prog:
                yield BoolIsUpper(prog1)


# Boolean classes and terminals

BOOL_TYPES = {'type': 'boolean', 'classes': (BoolLiteral, BoolEqual, BoolContain,
                                             BoolSuffixof, BoolPrefixof, BoolGreaterThan,
                                             BoolGreaterThanEqual,
                                             BoolAnd, BoolOr, BoolIsAlNum, BoolIsDigit,
                                             BoolIsLower, BoolIsUpper
                                             )}
