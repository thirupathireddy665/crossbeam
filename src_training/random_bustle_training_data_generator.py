...
def generate_training_data(program_synthesizer, gen_training_data_set, random_inputs,
    ...
    for program in random_programs:
        ...
        string_return_type_programs = list(
            filter(lambda child_program: child_program.getReturnType() == STR_TYPES['type'], child_programs_set))
        if len(string_return_type_programs) <= 0:
            continue
        positive_child = random.choice(string_return_type_programs)

        # FINDING NEGATIVE CHILD PROGRAM
        string_child_programs = []
        for child_size in range(2, 7):
            string_child_programs.extend(program_synthesizer.program_list.get_programs(child_size, STR_TYPES['type']))

        if len(string_child_programs) < 1:
            continue

        children = random.sample(string_child_programs, 2)
...
if __name__ == "__main__":
    ...
    log_filename = logs_directory + "random_bustle_training.log"
    os.makedirs(os.path.dirname(log_filename), exist_ok=True)

    training_data_filename = data_directory + "random_bustle_training_data.csv"
